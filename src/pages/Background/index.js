    ...
    chrome.tabs.get(activeTab, (tab) => {
        if (tab) {
            ...
            chrome.windows.update(tab.windowId, { focused: true }, async () => {
                ...
                const { countdown } = await chrome.storage.local.get([
                    'countdown',
                ])
                if (countdown) {
                    setTimeout(() => {
...
const onActivated = async (activeInfo) => {
    ...
    if (recording) {
        ...
        const { customRegion } = await chrome.storage.local.get([
            'customRegion',
        ])
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
    if (changeInfo.status === 'complete') {
        ...
        if (recordingStartTime) {
            ...
            if (alarm) {
                // Send remaining seconds
                const { alarmTime } = await chrome.storage.local.get([
                    'alarmTime',
                ])
                const seconds = parseFloat(alarmTime)
    ...
    const sendBatch = async (batch, retryCount = 0) => {
        try {
            ...
            if (retryCount < maxRetries) {
                console.error(
                    `Sending batch failed, retrying... Attempt ${
                        retryCount + 1
                    }`,
                    error
                )
...
const offscreenDocument = async (request, tabId = null) => {
    ...
    if (request.region) {
        ...
        try {
            ...
            const { qualityValue } = await chrome.storage.local.get([
                'qualityValue',
            ])
...
chrome.runtime.onInstalled.addListener(async (details) => {
    if (details.reason === 'install') {
        ...
        chrome.runtime.setUninstallURL(
            'https://godam.io/chrome-extension-feedback/'
        )
...
const handleSignOutGoDAM = async (sendResponse) => {
    ...
    if (godamToken) {
        ...
        try {
            await fetch(
                `${baseUrl}/api/method/frappe.integrations.oauth2.revoke_token`,
                {
                    method: 'POST',
                    headers: {
...
const refreshToken = async (sendResponse) => {
    const { godamRefreshToken } = await chrome.storage.local.get([
        'godamRefreshToken',
        'code',
        'redirectUrl',
    ])
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.type === 'desktop-capture') {
        ...
        createTab('https://app.godam.io/helpdesk/my-tickets', false, true)
...
chrome.cookies.onChanged.addListener(async (changeInfo) => {
    ...
    if (
        (cookie.name === 'sid' || cookie.name === 'user_id') &&
        cookie.value === 'Guest'
    ) {
        await chrome.storage.local.remove([
            'godamToken',
