...
const setOrgList = async () => {
  ...
  const response = await fetch(
    `${baseURL}/api/method/frappe_organization.api.manage.list_organizations`,
    {
      method: 'GET',
      headers: {
  ...
  const firstNonViewerOrg = data.message.find(
    (org) => org.role.toLowerCase() !== 'viewer',
  );

  if (firstNonViewerOrg) {
    await chrome.storage.local.set({
      selectedOrg: firstNonViewerOrg['organization_name'],
    });
  }
};
