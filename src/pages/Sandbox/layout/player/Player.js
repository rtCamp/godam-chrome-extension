...
const Player = () => {
    ...
    const signInGoDAM = async () => {
        try {
            ...
            const redirectUrl = chrome.identity
                .getRedirectURL()
                .replace(/\/$/, '')
            ...
            const token =
                ...
                (tokenData.message && tokenData.message.access_token)
            ...
            const expiresIn =
                ...
                (tokenData.message && tokenData.message.expires_in) ||
                3600
            const expirationTime = Date.now() + expiresIn * 1000
            const refreshToken =
                tokenData.refresh_token ||
                (tokenData.message && tokenData.message.refresh_token)
    ...
    const refreshToken = async () => {
        const token = await chrome.runtime.sendMessage({
            type: 'refresh-godam-token',
        })
        return token
    }

    const getGoDAMAuthToken = async () => {
        const { godamToken, godamTokenExpiration } =
    ...
    const saveToGoDAM = async () => {
        ...
        const uploadUrl =
            process.env.GODAM_UPLOAD_URL || 'https://upload.godam.io' // Todo: Replace the option with production URL
        ...
        if (!uploadResponse.ok) {
            ...
            if (uploadResponse.status === 400) {
                message =
                    'An error occurred while saving to GoDAM! <br> Looks like you are not logged in to GoDAM. Please log in again.'
            } else {
                message =
                    'An error occurred while saving to GoDAM! <br> Please try again later, if the problem persists, please contact <a href="https://app.godam.io/helpdesk/my-tickets" target="blank">support team</a>.'
            }
                ...
                <div className={styles.leftPanel}>
                    <div className={styles.saving}>
                        ...
                        <p className={styles.savingText}>
                            Hold tight! We're setting up your video...
                        </p>
                    </div>
                    <Content />
                </div>
